...
test('a comma-separated client key splits into req.geminiApiKeys, with req.geminiApiKey as the first', () => {
  const cfg = { geminiApiKey: undefined, proxyApiKey: undefined };
  const req = { headers: { authorization: 'Bearer key-one, key-two ,key-three' }, query: {} };
  const res = mockRes();

  resolveApiKey(req, res, () => {}, cfg);

  assert.deepEqual(req.geminiApiKeys, ['key-one', 'key-two', 'key-three']);
  assert.equal(req.geminiApiKey, 'key-one');
});

test('a comma-separated GEMINI_API_KEY (server fallback) also splits into req.geminiApiKeys', () => {
  const cfg = { geminiApiKey: 'fallback-one,fallback-two', proxyApiKey: undefined };
  const req = { headers: {}, query: {} };
  const res = mockRes();

  resolveApiKey(req, res, () => {}, cfg);

  assert.deepEqual(req.geminiApiKeys, ['fallback-one', 'fallback-two']);
  assert.equal(req.geminiApiKey, 'fallback-one');
});
