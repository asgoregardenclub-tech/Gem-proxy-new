...
export const config = {
  ...
  // Either of these — GEMINI_API_KEY here, or whatever's typed into
  // Janitor's own key field — may be a SINGLE key or a comma-separated
  // LIST of keys. With more than one key, see lib/keyManager.js: a
  // request that hits a dead/rate-limited/quota-exhausted key
  // automatically rotates to the next one before anything reaches
  // Janitor, instead of just failing.
  ...
  // How long a key sits out of rotation after a TRANSIENT failure (429 /
  // quota / RESOURCE_EXHAUSTED) before being tried again. Only matters
  // when more than one key is configured — see lib/keyManager.js.
  keyCooldownMs: Number(process.env.KEY_COOLDOWN_MS || 60_000),
  // How long a key sits out after looking PERMANENTLY dead (invalid,
  // revoked, UNAUTHENTICATED) — much longer than the above, since
  // retrying a truly bad key on every request just wastes an attempt.
  // Still not infinite, in case e.g. billing gets fixed without a
  // redeploy. Defaults to 24h.
  keyInvalidCooldownMs: Number(process.env.KEY_INVALID_COOLDOWN_MS || 24 * 60 * 60_000),

  // Per-category Gemini safety thresholds.
  // HARM_CATEGORY_JAILBREAK is deliberately NOT sent — that classifier is
  // off unless the category is present on the request.
  safetyThresholds: {
  ...
  // Short Gemini-native systemInstruction: fictional writing + every
  // character is 18+. Age disambiguation only, not a jailbreak. Disable
  // with ENABLE_ADULT_FRAMING=false. Override the text with ADULT_FRAMING_TEXT.
  enableAdultFraming: process.env.ENABLE_ADULT_FRAMING !== 'false',
  adultFramingText: process.env.ADULT_FRAMING_TEXT || undefined,
  ...
  // Frequency penalty (discourages tokens proportional to how often
  // they've already appeared) and presence penalty (a.k.a. "repetition
  // penalty" in most roleplay frontends — flat discouragement for any
  // token that's appeared at all, regardless of count). Same "only if
  // the client didn't already send one" rule; unset by default so
  // Janitor's own sliders always win when it sends them.
  defaultFrequencyPenalty: optionalNumber('DEFAULT_FREQUENCY_PENALTY'),
  defaultPresencePenalty: optionalNumber('DEFAULT_PRESENCE_PENALTY'),
