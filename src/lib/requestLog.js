...
export function logGeminiRequest({
  ...
  // ran out (raise max_tokens); 'SAFETY' means a configurable harm
  // category fired; 'PROHIBITED_CONTENT' is Google's non-configurable
  // policy filter (BLOCK_NONE cannot disable it).
  if (finishReason !== undefined) parts.push(`finishReason=${finishReason}`);
  if (promptBlockReason) parts.push(`promptBlockReason=${promptBlockReason}`);
