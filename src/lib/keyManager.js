// Multi-key rotation for the Gemini API key(s) a request ends up using.
// GEMINI_API_KEY (or whatever's typed into Janitor's own key field, via
// middleware/auth.js) may hold a single key or a comma-separated list —
// parseApiKeys() below splits it either way. routes/openaiCompat.js tries
// getCandidateKeys() in order for the upstream call; on a dead/rate-
// limited/quota-exhausted response it calls markKeyExhausted() and moves
// to the next candidate before anything reaches the client, and calls
// markKeySuccess() once one works so future requests start there too.
//
